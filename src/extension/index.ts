 ...
 *
 * Config file: ~/.pi/agent/extensions/fusion/config.json
 * Legacy subagent config is also supported at ~/.pi/agent/extensions/subagent/config.json.
...
import { registerFusionTool } from "./fusion.ts";
...
function loadConfig(): ExtensionConfig {
	const configPaths = [
		path.join(os.homedir(), ".pi", "agent", "extensions", "fusion", "config.json"),
	...
	for (const configPath of configPaths) {
		try {
			...
			console.error(`Failed to load Fusion/subagent config from '${configPath}':`, error);
		}
	}
	return {};
}
...
• { action: "doctor" } - read-only report for runtime paths, discovery, sessions, and intercom`,
	...
	registerFusionTool(pi, { executeSubagent: executeSubagentCollapsed });
