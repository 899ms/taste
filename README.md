...
Fusion is a Pi package forked from [`pi-subagents`](https://pi.dev/packages/pi-subagents). It keeps the full subagent system and adds a `fusion` tool for multi-model synthesis.

`fusion` asks multiple selected models the same prompt through child Pi sessions, shuffles and anonymizes their answers, then returns those anonymous candidate answers to the parent agent so it can synthesize one unbiased final response. The textual tool result never includes the model names; the model-to-candidate mapping is kept only in tool details for UI/audit.
...
Ask naturally and name the models you want to consult:

```text
Run Fusion on this architecture decision using anthropic/claude-opus-4-6, openai/gpt-5.5, and google/gemini-3-pro. Combine the best ideas into one recommendation.
```

Or call the tool shape directly:

```text
fusion({
  prompt: "Should we use SQLite or Postgres for this local-first app?",
...
- `models`: two or more `provider/model` ids; thinking suffixes like `:high` are allowed.
- `prompt`: exact prompt sent to every model.
- `style`: `balanced`, `concise`, `detailed`, or `decision`.
- `context`: `fresh` (default) or `fork`.
- `agent`: defaults to the built-in read-only `fusion-respondent` agent.
- `instructions`: extra per-panelist instructions.

## Subagents

Fusion includes the original `pi-subagents` feature set. Use it for code review, scouting, implementation, parallel audits, saved workflows, background jobs, and anything else that benefits from a second or third set of model eyes.
