...
`pipeline/taste/04-skill/SKILL.md` is Jaytel's example test skill generated by
this pipeline from a private reference-image corpus. Treat it as an example of
what the tool produces, not as the only possible taste. New reference images can
generate unlimited different taste skills.
...
## Hosted Web Demo Development

The hosted web demo uses OpenRouter OAuth only. Direct OpenAI/Anthropic
credentials are kept out of the hosted UI and public API.
