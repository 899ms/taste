...
### Added
- Forked `pi-subagents` into `pi-fusion`.
- Added the `fusion` tool for anonymous multi-model panel runs and parent-agent synthesis.
- Added the built-in read-only `fusion-respondent` agent.
- Updated package metadata, installer text, README, and parent orchestration skill guidance for Fusion.
