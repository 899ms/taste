...
name: pi-subagents
description: |
  Delegate work to builtin or custom subagents and use Fusion to ask multiple
  selected models the same prompt, anonymize their answers, and synthesize one
  final response. Supports single-agent, chain, parallel, async, forked-context,
  intercom-coordinated, and multi-model consensus workflows.
---

# Pi Subagents and Fusion

This skill is for the main parent orchestrator only. Do not inject or follow it inside spawned child subagents. The parent session owns delegation, orchestration, review fanout, Fusion synthesis, and final fix-worker launches; child subagents should receive concrete role-specific tasks and should not run their own subagent workflows.

Use this skill when the parent orchestrator needs to launch a specialized subagent, compose multiple agents into a workflow, create/edit agents and chains on demand, or run Fusion across several selected models.
...
- **Fusion / multi-model synthesis**: when the user asks to consult multiple named models and combine the best outputs, call `fusion(...)` with the exact prompt and model list, then synthesize the anonymous candidate answers without mentioning or guessing model identities

## Tool vs Slash Commands

Agents can use the `fusion(...)` tool for multi-model answer synthesis and the `subagent(...)` tool directly for execution, management, status, and control.
...
Use this when the user asks for Fusion, a multi-model consensus, to ask specific models the same question, or to combine outputs from models such as GPT, Claude, Gemini, etc. Call `fusion({ prompt, models, style? })` with the user's exact question/task and the selected model ids. The tool returns shuffled anonymous candidates, so the parent synthesizer must judge substance only.

Parent synthesis rules after `fusion(...)` returns:
- Do not mention or infer which model produced a candidate.
- Do not rank candidates by label/order; labels are arbitrary.
- Combine the strongest supported claims into one final answer.
- Preserve useful minority insights and caveats when they improve correctness.
- Resolve disagreements by reasoning quality, evidence, and fit to the user's prompt.
- If there is uncertainty, say so and give the safest recommendation.

Example:

```typescript
fusion({
  prompt: "Evaluate this API design and recommend a path forward: ...",
  models: ["anthropic/claude-opus-4-6", "openai/gpt-5.5", "google/gemini-3-pro"],
  style: "decision"
})
```
...
| `fusion-respondent` | Anonymous read-only Fusion panelist | set by Fusion call | Candidate answer for parent synthesis |
